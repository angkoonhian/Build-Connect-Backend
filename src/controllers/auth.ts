...
import { checkPassword, ErrorResponse, getSignedJwtToken, hashPassword } from '../utils';
...
export class AuthController {
    ...
    private sendTokenResponse = (user, statusCode, res, redirectHome = false) => {
        ...
        // Set cookie options
        const options = {
            expires: new Date(Date.now() + parseInt(process.env.JWT_COOKIE_EXPIRE, 10) * 24 * 60 * 1000),
        };
